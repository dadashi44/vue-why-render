...
| --- | --- | --- |
| `enabled` | `true` | Включает сканер. Режим сборки пакет не угадывает — см. [Стоимость](#стоимость). |
...
смешанный CommonJS и переписывает его в битый код. Поэтому сканер просто включён по умолчанию.
